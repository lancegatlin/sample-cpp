...
template<typename ConceptMatrix>
struct matrix_info {
    size_t rows(const ConceptMatrix &m) const;
    size_t columns(const ConceptMatrix &m) const;
};

template<unsigned long ROWS, unsigned long COLUMNS>
struct matrix_info<float_type[ROWS][COLUMNS]> {
    size_t rows(const float_type[ROWS][COLUMNS]) const { return ROWS; };
    size_t columns(const float_type [ROWS][COLUMNS]) const { return COLUMNS; };
};

template<typename ConceptMatrix>
size_t matrix_rows(const ConceptMatrix &m) { return matrix_info<ConceptMatrix>().rows(m); };

template<typename ConceptMatrix>
size_t matrix_columns(const ConceptMatrix &m) { return matrix_info<ConceptMatrix>().columns(m); };
