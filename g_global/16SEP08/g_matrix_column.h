...
template<class ConceptMatrix>
class MatrixConstColumn : public MatrixColumnBase<ConceptMatrix> {
    public:
    typedef MatrixColumn<ConceptMatrix> this_t;
    typedef MatrixColumnBase<const ConceptMatrix> parent_t;
    typedef const ConceptMatrix matrix_t;

    MatrixConstColumn(matrix_t *__matrix, size_t __column)
    : parent_t(__matrix,__column)
    { };
