#ifndef __G_MATRIX_ITERATOR_H
#define __G_MATRIX_ITERATOR_H
...
template<class ConceptMatrix>
class MatrixConstIterator : public MatrixIteratorBase<const ConceptMatrix> {
    public:
    typedef MatrixRow<const ConceptMatrix> this_t;
    typedef MatrixIterator<const ConceptMatrix> parent_t;
    typedef const ConceptMatrix matrix_t;
    
    MatrixConstIterator(matrix_t *__matrix = NULL, size_t __row = 0, size_t __column = 0)
    : parent_t(__matrix, __row, __column) 
    { }; 

    MatrixConstIterator(matrix_t &__matrix, size_t __row = 0, size_t __column = 0)
    : parent_t(__matrix, __row, __column) 
    { }; 
